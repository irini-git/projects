...
class EditConfusionMatrix:
    ...
    def get_probability(self):
        ...
        value01 = self.df.index[(self.df['correct'] == "e") & (self.df['incorrect'] == "ew")]

        # print(self.df.query('correct == "ca" & incorrect == "ac"'))
        value02 = self.df.index[(self.df['correct'] == "a") & (self.df['incorrect'] == "e")]

        # print(self.df.query('correct == "c" & incorrect == "r"'))
        value03 = self.df.index[(self.df['correct'] == "hr") & (self.df['incorrect'] == "h")]

        # print(self.df.query('correct == "o" & incorrect == "e"'))
        value04 = self.df.index[(self.df['correct'] == "we") & (self.df['incorrect'] == "ew")]
        ...
        # thwe      | we      | ew    |
        # -------------------------------
        candidate = ['thew', 'thaw', 'threw', 'thwe']
        self.df_subset.insert(0, 'candidate', candidate)

        # add original word 'the' with channel probability 0.95
        # the assumption of 1 typo in 20 from the lecture
        self.df_subset.loc[len(self.df_subset.index)] = ['the', '', '', 'none', .95]
    ...
    def calculate_word_model(self, text, column):
        ...
        possible_corrections = ['the', 'thew', 'thaw', 'threw', 'thwe']

        # Get log probability

        for w in possible_corrections:
            # Unigram model, Laplace (add one) smoothing
